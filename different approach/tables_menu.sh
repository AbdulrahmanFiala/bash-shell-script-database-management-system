	...
	then 
		...
		do
			...
			do
				...
				if [[ $colname == "int" || $colname == "varchar" ]]
				then
					echo "Column name cannot be 'int' or 'varchar'. Please choose a different name."
					continue
				fi
				...
				do
					...
					if [[ $pk == "Y" || $pk == "y" || $pk == "yes" ]]
					then
						# Change the value of the mark so it never enters the loop again if the user selects a field as PK 
						mark="false"
						# Use echo -n to avoid a newline and append (PK) to the field name
						echo -n "(PK)" >> $table
					else
						break
					fi
				...
				# If we reach here, the column name is valid, and we can break out of the loop
				break
			done
		...
		# If the user enters anything except numbers
